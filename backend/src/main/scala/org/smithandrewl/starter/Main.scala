...
import org.smithandrewl.starter.finch.endpoint.{AuthEndpoints, EventEndpoints, TaskEndpoints}
...
object Main extends TwitterServer  {
  ...
  def main() {
    ...
    val policy: Cors.Policy = Cors.Policy(
      ...
      allowsHeaders = _ => Some(Seq(Names.ACCEPT, Names.AUTHORIZATION, Names.ACCESS_CONTROL_ALLOW_CREDENTIALS, Names.CONTENT_TYPE, Names.ACCESS_CONTROL_ALLOW_ORIGIN))
    )

    val endpoints     = (AuthEndpoints.endpoints :+: EventEndpoints.endpoints :+: TaskEndpoints.endpoints).toService
